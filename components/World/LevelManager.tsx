...
export const LevelManager: React.FC = () => {
  ...
  useFrame((state, delta) => {
    ...
    for (const obj of currentObjects) {
        ...
        // Magnet attraction vacuum effect
        if (isMagnetActive && obj.type === ObjectType.GEM && obj.active) {
            const distZ = Math.abs(obj.position[2] - playerPos.z);
            if (distZ < 40 && obj.position[2] < playerPos.z) {
                obj.position[0] = THREE.MathUtils.lerp(obj.position[0], playerPos.x, safeDelta * 14);
                obj.position[1] = THREE.MathUtils.lerp(obj.position[1], playerPos.y + 0.6, safeDelta * 14);
                obj.position[2] = THREE.MathUtils.lerp(obj.position[2], playerPos.z, safeDelta * 14);
                hasChanges = true;
            }
        }
        ...
        if (obj.active) {
            ...
            if (obj.type === ObjectType.SHOP_PORTAL) {
                ...
                if (dx < 0.9) { // Slightly increased horizontal forgiveness
                     ...
                     const isDamageSource = obj.type === ObjectType.OBSTACLE || obj.type === ObjectType.ALIEN || obj.type === ObjectType.MISSILE || obj.type === ObjectType.LASER_BARRIER;
                     ...
                     if (isDamageSource) {
                         ...
                         if (obj.type === ObjectType.OBSTACLE) {
                             ...
                             objBottom = 0.5;
                             objTop = 1.5;
                          } else if (obj.type === ObjectType.LASER_BARRIER) {
                              objBottom = 0;
                              objTop = 3.2;
                         }

                         let isLaserInactive = false;
                          if (obj.type === ObjectType.LASER_BARRIER) {
                              const active = Math.floor(state.clock.elapsedTime * 2.5) % 2 === 0;
                              if (!active) {
                                  isLaserInactive = true;
                              }
                          }
                          const isHit = !isLaserInactive && (playerBottom < objTop) && (playerTop > objBottom);
                         ...
                         if (dy < 2.5) { // Generous vertical pickup range
                            ...
                            if (obj.type === ObjectType.SHIELD) {
                                 activateShield();
                                 audio.playLetterCollect();
                             }
                             if (obj.type === ObjectType.MAGNET) {
                                 activateMagnet();
                                 audio.playLetterCollect();
                             }
                             if (obj.type === ObjectType.BOOST) {
                                 activateBoost();
                                 audio.playLetterCollect();
                             }
    ...
    if (furthestZ > -SPAWN_DISTANCE) {
         ...
         if (isLetterDue) {
            ...
            if (isObstacle) {
                ...
                const spawnAlien = level >= 2 && spawnRoll < 0.20;
                const spawnLaser = spawnRoll >= 0.20 && spawnRoll < 0.35; // 15% chance of spawning laser gates
                ...
                // Decide between GEM or POWER-UP
                const isPowerup = Math.random() < 0.22; // 22% chance of a power-up spawning
                const lane = getRandomLane(laneCount);
                
                if (isPowerup) {
                    const pRoll = Math.random();
                    let pType = ObjectType.SHIELD;
                    let pColor = '#3dd1ff';
                    
                    if (pRoll < 0.33) {
                        pType = ObjectType.SHIELD;
                        pColor = '#3dd1ff'; // Cyan matrix shield
                    } else if (pRoll < 0.66) {
                        pType = ObjectType.MAGNET;
                        pColor = '#00ffd2'; // Neon green coin magnet
                    } else {
                        pType = ObjectType.BOOST;
                        pColor = '#ffff00'; // Flash gold speed booster
                    }
...
const GameEntity: React.FC<{ data: GameObject }> = React.memo(({ data }) => {
    ...
    const isFlashingActive = Math.floor(Date.now() / 250) % 2 === 0;
                ...
                {data.type === ObjectType.SHIELD && (
                    <group>
                        <mesh>
                            ...
                            <meshStandardMaterial color="#3dd1ff" transparent opacity={0.5} wireframe />
                        </mesh>
                        <mesh>
                            <octahedronGeometry args={[0.22]} />
                            <meshStandardMaterial color="#00ffff" emissive="#00ffff" emissiveIntensity={3} metalness={0.9} roughness={0.1} />
                        </mesh>
                    </group>
                )}
                ...
                {data.type === ObjectType.MAGNET && (
                    <group>
                        <mesh>
                            ...
                            <meshStandardMaterial color="#00ffd2" emissive="#00ffd2" emissiveIntensity={3} />
                        </mesh>
                ...
                {data.type === ObjectType.BOOST && (
                    <group rotation={[Math.PI / 2, 0, 0]}>
                        ...
                        <mesh position={[0, -0.28, 0]}>
                            ...
                            <meshStandardMaterial color="#ff9900" emissive="#ff9900" emissiveIntensity={3} />
                        </mesh>
                    </group>
                )}

                {/* --- LASER BARRIER HAZARD --- */}
                {data.type === ObjectType.LASER_BARRIER && (
                    <group>
                        {/* Anchors */}
                        <mesh position={[-1.0, 0, 0]}>
                            <cylinderGeometry args={[0.1, 0.12, 3.2, 8]} />
                            <meshStandardMaterial color="#222225" metalness={0.8} roughness={0.2} />
                        </mesh>
                        <mesh position={[1.0, 0, 0]}>
                            <cylinderGeometry args={[0.1, 0.12, 3.2, 8]} />
                            <meshStandardMaterial color="#222225" metalness={0.8} roughness={0.2} />
                        ...
                        <mesh position={[0, 1.5, 0]} rotation={[0, 0, Math.PI / 2]}>
                            ...
                            <meshStandardMaterial color="#111112" metalness={0.9} roughness={0.1} />
                        </mesh>
                        {/* Laser Sheet */}
                        <mesh position={[0, 0, 0]}>
                            <boxGeometry args={[1.8, 2.8, 0.05]} />
                            <meshBasicMaterial 
                                color={isFlashingActive ? "#ff1744" : "#ff1744"} 
                                transparent 
                                opacity={isFlashingActive ? 0.75 : 0.05} 
                            />
                        </mesh>
                    </group>
                )}
