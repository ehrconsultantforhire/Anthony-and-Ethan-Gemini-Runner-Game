...
interface GameState {
  ...
  // Character and Mobile Settings
  character: 'anthony' | 'ethan';
  ...
  // Character and Mobile Settings Actions
  setCharacter: (char: 'anthony' | 'ethan') => void;
...
export const useStore = create<GameState>((set, get) => ({
  ...
  takeDamage: () => {
    const { lives, isImmortalityActive, vibrationEnabled, triggerVibration, isShieldActive, isBoostActive } = get();
    if (isImmortalityActive || isBoostActive) return; // No damage if skill or booster is active

    if (isShieldActive) {
      set({ isShieldActive: false }); // Consume shield
      if (vibrationEnabled) {
    ...
    if (vibrationEnabled) {
      triggerVibration(200); // Vibrate on damage
    }
  ...
  collectGem: (value) => {
    const { vibrationEnabled, triggerVibration } = get();
    if (vibrationEnabled) {
      triggerVibration(30); // Very brief vibration for collecting gem
    }
    set((state) => ({ 
  ...
  collectLetter: (index) => {
    const { collectedLetters, level, speed, vibrationEnabled, triggerVibration } = get();
    
    if (vibrationEnabled) {
      triggerVibration(80); // Nice vibration for letter colelct
    }
  ...
  activateMagnet: () => {
    ...
    }, 10000); // Magnet lasts 10 seconds!
  },

  activateBoost: () => {
    const { speed } = get();
    set({ isBoostActive: true, speed: speed * 1.5 });
    setTimeout(() => {
      const currentSpeed = get().speed;
      // Bring back down safely
      set({ isBoostActive: false, speed: Math.max(RUN_SPEED_BASE, currentSpeed / 1.5) });
    }, 6000); // Speed boost lasts 6 seconds!
  },
